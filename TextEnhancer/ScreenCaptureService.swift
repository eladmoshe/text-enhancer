...
class ScreenCaptureService {
    ...
    func captureActiveScreen() -> NSImage? {
        ...
        try? debugMessage.appendingFormat("").write(to: URL(fileURLWithPath: debugPath), atomically: false, encoding: .utf8)
        ...
        guard let activeScreen = getActiveScreen() else {
            ...
            try? errorMsg.appendingFormat("").write(to: URL(fileURLWithPath: debugPath), atomically: false, encoding: .utf8)
            return nil
        }
        ...
        try? screenMsg.appendingFormat("").write(to: URL(fileURLWithPath: debugPath), atomically: false, encoding: .utf8)
    ...
    func convertImageToBase64(_ image: NSImage, quality: CGFloat = 0.7) -> String? {
        ...
        print("🔧 ScreenCaptureService: Bitmap created - size: \(bitmap.size), pixels: \(bitmap.pixelsWide)x\(bitmap.pixelsHigh)")
