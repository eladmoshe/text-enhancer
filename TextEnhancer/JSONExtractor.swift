...
extension JSONParseError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .invalidUTF8:
            "⚠️ Invalid response encoding from API"
        case .noJSONFound:
            "⚠️ API returned unexpected response format"
        case let .invalidJSON(error):
            "⚠️ Invalid response format: \(error.localizedDescription)"
        case let .missingRequiredField(field):
            "⚠️ Incomplete response from API (missing: \(field))"
        }
    }
}
                ...
                while currentIndex < response.endIndex {
                    ...
                    if char == "{" {
                        ...
                        if braceCount == 0 {
                            ...
                            let jsonCandidate = String(response[jsonRange])
