...
class RetryController {
    ...
    func delay(forAttempt attempt: Int) -> TimeInterval {
        switch attempt {
        case 1: 0.0 // First retry immediately
        case 2: 2.0 // Second retry after 2 seconds
        default: 0.0 // No more retries
        }
    }
...
struct AlertAction {
    ...
    static func openSystemSettings(panel: String = "") -> AlertAction {
        AlertAction(title: "Open System Settings") {
            let url = if panel.isEmpty {
                "x-apple.systempreferences:"
            } else {
                "x-apple.systempreferences:\(panel)"
            }
    ...
    static func copyError(_ errorDetails: String) -> AlertAction {
        AlertAction(title: "Copy Error Details") {
...
class DefaultTextSelectionProvider: TextSelectionProvider {
    func getSelectedText() -> String? {
        ...
        guard result == .success, let focusedElement,
...
class DefaultAccessibilityChecker: AccessibilityChecker {
    func isAccessibilityEnabled() -> Bool {
        ...
        if isTrusted {
            ...
            if result != .success {
                print(
                    "🔐 AccessibilityChecker: AXIsProcessTrusted=true but can't access focused element (result: \(result.rawValue))"
                )
                return false
            }
        }
...
protocol APIProviderService {
    ...
    func enhanceText(_ text: String, with prompt: String, using model: String, screenContext: String?) async throws
        -> String
}
...
class TextProcessor: ObservableObject {
    ...
    func processSelectedText(with prompt: String) async {
        ...
        let model = shortcut?.effectiveModel ?? "claude-sonnet-4-20250514"
    ...
    func processSelectedText(with prompt: String, using provider: APIProvider, model: String) async {
        ...
        let processingTask = Task {
            ...
            do {
                ...
                \(configManager.configuration.shortcuts
                    .map { "🔧   - ID: '\($0.id)', Name: '\($0.name)', Prompt: '\($0.prompt)'" }.joined(separator: "\n"))
                🔧 TextProcessor: Found shortcut: \(shortcut?.id ?? "none"), name: \(shortcut?
                    .name ?? "none"), includeScreenshot: \(shortcut?.effectiveIncludeScreenshot ?? false)
                """
                print(debugMsg)
                ...
                print(
                    "🔧 TextProcessor: All shortcuts in config: \(configManager.configuration.shortcuts.map { "\($0.id):\($0.name)" })"
                )
                ...
                print(
                    "🔧 TextProcessor: DEBUG - effectiveIncludeScreenshot: \(foundShortcut?.effectiveIncludeScreenshot ?? false)"
                )
                ...
                if let screenContext {
                    ...
                    enhancedText = try await apiService.enhanceText(
                        selectedText,
                        with: prompt,
                        using: model,
