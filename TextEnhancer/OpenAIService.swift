...
class OpenAIService: ObservableObject {
    ...
    private func enhanceTextWithRetry(
        ...
        while retryController.hasAttemptsRemaining {
            ...
            do {
                ...
                print(
                    "⚠️ OpenAIService: Attempt \(attempt) failed, retrying in \(retryController.delay(forAttempt: attempt))s: \(error)"
                )
        ...
        if let screenContext {
            ...
            let messageContent = [
                OpenAIMessageContent(
                    type: "image_url",
                    image_url: OpenAIImageURL(url: "data:image/jpeg;base64,\(screenContext)", detail: "high")
                ),
    ...
    func fetchAvailableModels() async throws -> [OpenAIModel] {
        ...
        print(
            "✅ OpenAIService: Fetched \(modelsResponse.data.count) models, filtered to \(sortedModels.count) relevant models"
        )
